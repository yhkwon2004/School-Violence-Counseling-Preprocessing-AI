...
2. 사건 메모를 쓰고 앱을 다시 열어 SecureStore 초안 복구를 확인한다. 제출 뒤 앱을 다시 열어 이전 write가 초안을 되살리지 않는지도 확인한다.
3. 이미지 또는 PDF를 올리고 즉시 다음 단계로 이동한다.
4. 처리 상태가 Realtime 또는 polling fallback으로 갱신되는지 확인한다.
5. 증거가 대기 또는 처리 중이면 확인 화면의 제출 버튼이 잠기는지 확인한다.
6. 누락 질문을 수정하는 동안 다음 이동이 잠기고, 저장 완료 뒤 풀리는지 확인한다. 저장 실패를 재현할 수 있다면 재시도와 변경 취소도 확인한다.
7. 질문 답변 저장과 새 FactBlock 확인 뒤 제출한다. 제출 연속 탭이 중복 요청을 만들지 않고 제출 뒤 메모 수정과 추가 업로드가 잠기는지 확인한다.
8. 앱을 다시 열어 제출 사건 잠금 화면이 복원되는지 확인한다.
9. 상담자 웹 재개방 뒤 앱의 확인 버튼으로 수정 흐름에 돌아가는지 확인한다.
10. 잠금 화면에서 새 기록 작성을 명시적으로 선택할 수 있는지 확인한다.
11. 삭제 요청 확인창에서 취소하면 유지되고, 확인하면 사건이 즉시 숨겨지는지 확인한다.
