...
| Android 기기·OS | 미검증 |
| Expo Go 버전 | 미검증 |
| 개발 PC LAN 주소 | 자동 확인: `172.30.1.51` |
| Expo Go URL | 자동 확인: `exp://172.30.1.51:8081` |
| 커밋 또는 작업트리 기준 | 로컬 미커밋 작업트리 |
...
| 단계 | 확인 내용 | 결과 | 관찰 기록 |
| --- | --- | --- | --- |
| 1 | 익명 ID와 비밀번호로 로그인 | 미검증 |  |
| 2 | 안전 안내 확인 뒤 사건 메모 작성 | 미검증 |  |
| 3 | 앱을 닫고 다시 열어 SecureStore 텍스트 초안·현재 단계 복구, 오래된 기기 초안의 서버 메모 덮어쓰기 방지 | 미검증 |  |
| 4 | 이미지 또는 PDF 업로드 뒤 즉시 다음 단계 이동 | 미검증 |  |
| 5 | Realtime 또는 polling fallback으로 처리 상태 갱신 | 미검증 |  |
| 6 | 증거 대기·처리 중 확인 화면의 제출 버튼 잠금 | 미검증 |  |
| 7 | 누락 질문 답변 입력 즉시 표시, polling 중 입력 유지, 저장과 FactBlock 후보 확인 | 미검증 |  |
