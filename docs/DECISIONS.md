...
- 코드는 학생 앱에서 제출 완료 뒤 생성하는 사건 인계 코드로 정의한다.
- 관리자 발급 로그인 코드나 외부기관 공유 링크와 섞지 않는다.
- 평문 코드는 저장하지 않고 `HANDOFF_CODE_PEPPER`와 정규화 코드의 해시만 저장한다.
- 새 코드를 만들면 이전 미사용 코드는 폐기하고, redeem은 Postgres RPC에서 행 잠금으로 1회만 처리한다.
- 상담자는 제출 대기 사건만 코드로 원자적으로 가져오고, 기관 관리자는 사건을 찾은 뒤 배정한다.

### Precise SVG relation graph

- 관계도는 고정 CSS 선을 사용하지 않고 `RelationGraphLayout` 순수 계산 결과를 SVG로 렌더링한다.
- 화살표는 노드 중심이 아니라 원형 노드 경계에 붙인다.
- 간접 관계는 점선으로 표시하고, 관계 라벨은 간선 중앙에 둔다.
- 상담자·관리자는 자동 배치 뒤 노드 위치를 드래그로 보정하고 `people.position_x/y`에 저장한다.
- 노드나 간선을 클릭하면 선택 요소와 1차 연결 관계만 포커싱하는 사건철 모달을 연다.

### Internal casefile first

- "수사기관 자료처럼" 보이는 화면은 상담 전처리용 내부 사건철 UI로 제한한다.
- PDF 요약에는 관계도, FactBlock, 증거, 확인 필요 항목을 넣되 법률 판단과 공식 수사기관 양식 표현은 넣지 않는다.
- 외부기관 읽기 전용 공유 코드는 보안·법률 검토가 필요한 다음 단계로 문서화만 한다.
...
- 수정 중·저장 중·실패 답변이 남아 있으면 다음 단계와 제출을 잠근다. 실패 답변은 재시도하거나 마지막 서버 값으로 되돌려 선택형 질문 흐름을 계속할 수 있다.
- 메모 재분석을 시작하면 이전 질문 초안을 비워 새 FactBlock 질문에 오래된 답변을 재사용하지 않는다.

### Memo background saves keep request order

- 학생 메모 자동 저장 PATCH는 `MemoUpdateQueue` 객체 하나를 통해 순서대로 실행한다.
