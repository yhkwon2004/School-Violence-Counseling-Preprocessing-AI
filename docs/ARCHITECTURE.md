...
- 모바일 질문 카드는 제어형 로컬 답변 초안을 사용한다. polling으로 질문 목록을 복원할 때 새 질문은 서버 답변으로 초기화하고, 같은 질문의 작성 중 로컬 답변은 유지하며 사라진 질문 초안과 저장 상태를 정리한다. 입력 변경과 저장 시작은 질문별 요청 버전을 올린다. 늦게 끝난 이전 요청의 결과는 버리고 최신 상태만 표시한다. 수정 중·저장 중·실패 답변이 있으면 다음 단계와 제출을 잠근다. 저장 실패 시 카드 안에서 입력을 유지해 재시도하거나 서버 값으로 되돌릴 수 있다. 메모 재분석을 시작할 때는 이전 질문 초안을 비운다.
- `StudentApiClient`는 `MemoUpdateQueue` 객체로 background 메모 PATCH를 순서대로 처리한다. 분석 요청은 최신 메모 PATCH를 큐에 넣고 기다리며, 제출 요청은 남은 큐가 끝난 뒤 트랜잭션 RPC를 호출한다. 모바일 화면도 별도 `MemoUpdateQueue`로 SecureStore 초안 write·clear 순서를 보장한다. 제출 시작은 React 렌더보다 먼저 적용되는 동기 ref 가드로 연속 탭을 차단한다. 제출 중에는 새 자동 저장 예약을 멈추고 서버 제출 성공 뒤 clear를 기존 write 뒤에 배치한다.
- 배포 readiness는 모바일 코어 테스트의 `memoUpdateQueue.test.ts` 포함, API의 큐 대기, 질문 미저장 잠금, SecureStore 큐, 동기 제출 ref, 키보드 완료 저장 동작을 clean checkout에서도 정적으로 확인한다.
- 연결 웹 UI 스모크는 상담자, 기관 관리자, 플랫폼 관리자 seed 계정을 순서대로 로그인해 역할별 화면 경계를 확인한다. 상담자 FactBlock-증거 연결, 기관 관리자 보관 기간, 플랫폼 관리자 기관 관리 메뉴를 검사하고 실패 시 마지막 화면을 남긴다.
- 기관 관리자 보관 정책 grid의 입력은 컨테이너 너비 안에서 축소되고 작은 viewport에서는 한 열로 접힌다.
